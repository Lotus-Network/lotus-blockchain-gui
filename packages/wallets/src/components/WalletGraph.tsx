...
import { TransactionType } from '@lotus/api';
import type { Transaction } from '@lotus/api';
import { mojoToLotus, blockHeightToTimestamp } from '@lotus/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToLotus(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToLotus(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToLotus(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToLotus(start).toString, // bignumber is not supported by react
    });
  });
