...
import { OfferSummaryRecord, OfferTradeRecord } from '@lotus/api';
import {
  Box,
  Checkbox,
  Chip,
...
import { Offers } from '@lotus/icons';
import {
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: lchFee } = methods.getValues();

    const fee = cancelWithTransaction ? lotusToMojo(lchFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TLCH';
        ...
        <Grid container>
          <Grid xs={12} md={6} lg={5} item>
            <CardHero>
              ...
              <Typography variant="body1">
                <Trans>
                  Create an offer to exchange assets including LCH, tokens, and
