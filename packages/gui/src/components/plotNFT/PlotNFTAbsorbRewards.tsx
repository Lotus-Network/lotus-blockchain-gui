...
import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@lotus/api-react'
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const { fee } = data;
      const feeMojos = lotusToMojo(fee);
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={mojoToLotusLocaleString(balance)}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {address}
