...
const getLotusVersion = () => {
  let version = null;
  const exePath = getExecutablePath('lotus');
  // first see if we can get a lotus exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or lotus is on the path
    try {
      version = child_process
